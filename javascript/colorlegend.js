...
var colorlegend = function (target, scale, type, options) {
    ...
    var scaleTypes = ['linear', 'quantile', 'ordinal']
        ...
        , isVertical = opts.isVertical || true    // the legend is vertical
        ...
        , domain = scale.domain()
        , range = scale.range()
        , labels = opts.labels || function(d) { return d; }
        , i = 0;
    ...
    if (!isVertical) {
        ...
        // is vertical
        if (fill || w < boxWidth + padding[1] + padding[3]) {
            boxWidth = (w - padding[1] - padding[3]);
        }
        if (fill || h < (boxHeight + boxSpacing) * colors.length + padding[0] + padding[2] + titlePadding) {
            boxHeight = (h - padding[0] - padding[2] - (boxSpacing * colors.length) - titlePadding) / colors.length;
        }
    }
    ...
    var legendBoxes = legend.selectAll('g.legend')
        .isolatesCrossfilter(colors)
        .enter().append('g');

    var yOrigin = titlePadding + (colors.length * (boxHeight + boxSpacing));
...
//        .attr('dy', '.71em')
        ...
        .attr('y', function (d, i) {
            if (!isVertical) {
                ...
                return yOrigin -
                    (i * (boxHeight + boxSpacing)) + (boxHeight / 2) + 2;
            }
        })
        .style('text-anchor', function () {
            return type === 'ordinal' || isVertical ? 'start' : 'middle';
        })
        ...
        .text(function (d, i) {
            ...
            if (type === 'ordinal') {
                return labels(domain[i]);
            } else if (type === 'quantile') {
                var q = scale.quantiles()[i];
                if (q === undefined) {
                    q = scale.domain()[1];
                }
                return labels(q);
            ...
            // show only the first and last for others
                ...
                if (i === 0)
                    return labels(domain[0]);
                if (i === colors.length - 1)
                    return labels(domain[domain.length - 1]);
            }
        });
    ...
    legendBoxes.append('rect')
        ...
        .attr('y', function (d, i) {
            if (!isVertical) {
                ...
                return yOrigin - (i * (boxHeight + boxSpacing));
            }
        })
        .attr('width', boxWidth)
    ...
    if (title) {
        legend.append('text')
            ...
            .attr('x', (!isVertical ? (colors.length * (boxWidth / 2)) : boxSpacing))
            .attr('y', (!isVertical ? boxHeight + titlePadding : titlePadding ))
            .attr('dy', '.71em')
            .style('text-anchor', (!isVertical ? 'middle' : 'left'))
            .style('pointer-events', 'none')
            .text(title);
    }
